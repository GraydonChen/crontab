...
if (TSWOOLE_ENV === 1) {
	$SwooleConfig = include dirname(__FILE__) . "/Config/Crontab/config.on{$slave}.php";
} else {
	$SwooleConfig = include dirname(__FILE__) . "/Config/Crontab/config.demo{$slave}.php";
}
$SwooleConfig['MainProcessName'] = implode(' ', $argv);
$SwooleConfig['Port'] = TSWOOLE_PORT;
$SwooleConfig['SocketType'] = SWOOLE_SOCK_TCP;
$SwooleConfig["Behavior"] = array("CrontabBehivor", TSWOOLE_WEBROOT . 'crontab/Swoole/Apps/Crontab/CrontabBehivor.php');
...
$CrontabService = new SwooleService($SwooleConfig);
...
function processEnd() {
	...
	if ($error) {
		...
		if (isset($GLOBALS['crontab_method'])) {
			$error['crontab_method'] = $GLOBALS['crontab_method'];
		}
		Swoole_Log('swoole_crontab_run_error', var_export($error, true));
	}
}
