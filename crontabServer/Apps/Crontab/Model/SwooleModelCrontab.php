...
/**
 * Description of ManagerControoler
 *
 * @author JsonChen
 */
class SwooleModelCrontab {
	...
	public static function  GetMonitorInfo($action) {
		...
		$ret['logList'] = $logList;
		$ret['status'] = SwooleHelper::I()->Swoole->stats();
		...
		$write->WriteString(json_encode($ret));
	...
	public static function SaveMonitorInfoToLocal() {
		$load = sys_getloadavg();
		$local_ip = SwooleHelper::Get_Local_Ip();
		ocache::redis()->hSet("system_getloadavg", $local_ip, $load);
		global $monitor_table;
		$logList = array();
		$methods = array();
	...
	public static function SendMonitorInfo($msg) {
		oo::logs()->debug('时间:' . date('Ymd H:i:s') . $msg, 'swoole_monitor.txt');
	}
	...
	public static function err_decrCrontabCnt($crontab_name, $data = array(), $workid = 0, $time = 0) {
		...
		$log = array(
			...
			'curr_time' => date('Ymd H:i:s'),
			'workid' => $workid,
		);
		$log['data'] = $data;
		oo::logs()->debug($log, 'swoole_decrCrontabCnt.log');
	}
	...
	public static function GetCrontabConfig($type) {
		static $cfg = false;
		if (!$cfg) {
			include_once SWOOLE_CRONTAB_ROOT . 'Model/SwooleCrontabConfig.php';
			if (TSWOOLE_MAIN) {
				$cfg[self::ProcessListName] = SwooleCrontabConfig::getMainRunList(TSWOOLE_SID, TSWOOLE_ENV === 1);
			} else {
				$cfg[self::ProcessListName] = SwooleCrontabConfig::getSlaveRunList(TSWOOLE_SID, TSWOOLE_ENV === 1);
			}
			$cnt = 0;
	...
	public static function CrontabProcess($serv, $name) {
		...
		if(is_array($process['runTime']) && !self::checkRunTime($process['runTime'])){//检查时间是否满足
			return;
		}
		//获取当前进程的进程数限制配置
		...
		while (1) {
			...
			if ($run_all_max_cnt < $cfg_all_max_cnt && $run_process_max_cnt < $cfg_process_max_cnt && $cnt <= $cfg_process_max_cnt) {
				if(!PRODUCTION_SERVER && strpos($name,'genTableList')!==false){
					oo::logs()->debug(date('Ymd H:i:s').'--'.$process['method'],$name.'_swoole.txt');
				}
